...
export async function middleware(request: NextRequest) {
  ...
  if (user) {
    ...
    try {
      // Use database function for consistent trial status checking
      const { data: hasActiveAccess, error: accessError } = await supabase
        .rpc('has_active_access', { user_id: user.id })

      if (accessError) {
        console.error('Middleware: Error checking access via database function:', accessError)
        ...
        const hasAccess = hasActiveAccess || false
