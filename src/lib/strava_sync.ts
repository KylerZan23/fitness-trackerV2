  ...
  try {
    ...
    if (syncMode === 'recent') {
      ...
      if (lastSyncedActivity?.start_date) {
        ...
        console.log(
          `Syncing activities after last synced date: ${new Date(afterTimestamp * 1000).toISOString()}`
        )
      } else {
      ...
      if (activities.length > 0) {
        ...
        if (upsertError) {
          ...
          return {
            ...
            newActivitiesCount: totalNewActivitiesCount,
            error: 'Database upsert error: ' + upsertError.message,
            lastActivityDate: lastProcessedActivityDate,
          }
        }
        ...
        console.log(
          `Page ${page}: Upserted ${count ?? 0} activities. Total synced this job: ${totalSyncedCount}.`
        )
    ...
    if (pagesFetched >= MAX_PAGES_PER_SYNC_JOB) {
      console.warn(
        `Reached max pages (${MAX_PAGES_PER_SYNC_JOB}) for sync job. More activities might be available.`
      )
    }

    console.log(
      `Sync finished for user ${userId.substring(0, 6)}. Total activities processed/synced in this job: ${totalSyncedCount}. New: ${totalNewActivitiesCount}`
    )
    return {
      syncedCount: totalSyncedCount,
      newActivitiesCount: totalNewActivitiesCount,
    ...
    if (error.message && error.message.includes('Rate Limit Exceeded')) {
      return {
        syncedCount: totalSyncedCount,
        newActivitiesCount: totalNewActivitiesCount,
        error: 'Strava API rate limit exceeded during sync.',
        lastActivityDate: lastProcessedActivityDate,
      }
    }
    return {
