...
export const getStravaAuthUrl = (): string => {
  if (!STRAVA_CLIENT_ID) {
    throw new Error('NEXT_PUBLIC_STRAVA_CLIENT_ID environment variable is not set')
  }

  const redirectUri = STRAVA_REDIRECT_URI || `${window.location.origin}/run-logger/callback`
...
export const exchangeStravaCode = async (code: string): Promise<StravaTokens> => {
  if (!STRAVA_CLIENT_ID || !STRAVA_CLIENT_SECRET) {
    throw new Error('Strava client credentials are not properly configured')
  }
