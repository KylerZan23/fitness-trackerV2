...
export async function triggerStravaSync(
  ...
  if (!tokens) {
    ...
    return { syncedCount: 0, newActivitiesCount: 0, error: 'Strava not connected or tokens missing. Please reconnect Strava.' };
  }
  ...
  if (result.error) {
    ...
    console.log(`Strava sync completed for user ${user.id.substring(0,6)}. Synced: ${result.syncedCount}, New: ${result.newActivitiesCount}`);
