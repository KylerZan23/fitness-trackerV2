...
export default function MonthlyWorkoutsPage({
  params,
}: {
  params: Promise<MonthlyWorkoutsPageParams>
}) {
  ...
  const calendarCells = useMemo(() => {
    ...
    for (let i = 0; i < startingDayOfWeek; i++) {
      cells.push(
        <div
          key={`pad-start-${i}`}
          className="border-b border-r border-gray-100 h-32 bg-gray-50"
        ></div>
      )
    }
    // Day cells
    for (let day = 1; day <= daysInCurrentMonth; day++) {
      ...
      cells.push(
        <div
          key={`day-${day}`}
          className="border-b border-r border-gray-200 h-32 p-2 relative flex flex-col bg-white"
        >
    ...
    <DashboardLayout sidebarProps={sidebarProps}>
      ...
      <div className="grid grid-cols-7 border-t border-l border-gray-200 bg-gray-200">
        ...
        {daysOfWeek.map(day => (
          <div
            key={day}
            className="text-center p-2 text-xs font-medium text-gray-500 uppercase border-b border-r border-gray-200 bg-gray-50"
          >
            {day}
          </div>
        ))}
