...
import { getAllWorkouts, getUserProfile, HistoricalWorkout, getLocalStravaRunsForYear } from '@/lib/db'
...
export default function WorkoutsHistoryPage() {
  ...
  const handleStravaSync = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        setSyncStravaMessage(`Sync complete! Synced: ${result.syncedCount} activities. New: ${result.newActivitiesCount}. Last activity processed: ${result.lastActivityDate ? new Date(result.lastActivityDate).toLocaleDateString() : 'N/A'}. Data should refresh.`);
  ...
  const combinedWorkouts = useMemo<HistoricalWorkout[]>(() => {
    console.log(`Combining ${allLiftingWorkouts.length} lifting workouts and ${localStravaRuns.length} local Strava runs.`);
  ...
  const groupedWorkouts = useMemo<GroupedWorkouts>(() => {
    ...
    for (const workout of currentCombinedWorkouts) {
      try {
        ...
        console.error(`Error parsing date for workout ID ${workout.id} (${workout.type}): ${workout.created_at}`, parseError);
      }
    }
    ...
    return (
      <DashboardLayout sidebarProps={sidebarProps}>
        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm mb-6">
           <h1 className="text-2xl font-semibold mb-4 text-destructive">Error Loading Workout History</h1>
  ...
  return (
    <DashboardLayout sidebarProps={sidebarProps}>
      ...
      {syncStravaMessage && (
        <div className={`p-3 mb-4 rounded-md text-sm ${syncStravaMessage.includes('failed') || syncStravaMessage.includes('Error') ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>
          {syncStravaMessage}
        </div>
      )}
