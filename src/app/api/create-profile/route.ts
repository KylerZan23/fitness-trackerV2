...
export async function POST(request: NextRequest) {
  try {
    ...
    if (auth_token) {
      console.log(
        `API - Profile Creation: Starting token verification with token length: ${auth_token.length}`
      )
      console.log(
        `API - Profile Creation: First 10 chars of token: ${auth_token.substring(0, 10)}...`
      )
      ...
      try {
        ...
        console.log(
          `API - Profile Creation: Token expiration: ${new Date(decodedToken.exp * 1000).toISOString()}`
        )
        ...
        console.log(
          `API - Profile Creation: User from admin.getUserById:`,
          userData.user
            ? JSON.stringify({
                id: userData.user.id,
        ...
        if (userData.user.id !== id) {
          console.error(
            `API - Profile Creation: User ID mismatch - Token user: ${userData.user.id}, Request ID: ${id}`
          )
          return NextResponse.json(
