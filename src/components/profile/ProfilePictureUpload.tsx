...
export const ProfilePictureUpload = ({ userId, existingUrl, onUploadComplete }: ProfilePictureUploadProps) => {
  ...
  const fixSchema = async () => {
    try {
      ...
      if (alterError) {
        ...
        if (updateError) {
          if (updateError.message && updateError.message.includes('does not exist')) {
            throw new Error('Database column does not exist. Please run the SQL in the docs/profile-picture-fix.md file.')
          }
          throw updateError
        }
      }
      ...
      let errorMessage = 'Failed to fix database schema. Please run the SQL in docs/profile-picture-fix.md.'
