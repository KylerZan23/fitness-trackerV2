...
/**
 * AI Weekly Review content component without Card wrapper
...
export function AIWeeklyReviewContent() {
  ...
  const handleFollowUpSubmit = async () => {
    if (!followUpQuestion.trim() || !reviewData || followUpLoading) return
    ...
    try {
      ...
      setFollowUpError('Unable to get a response. Please try again.')
    } finally {
          ...
          <div className="mt-4 space-y-4">
                ...
                {followUpHistory.map((qa, index) => (
                  <div key={index} className="space-y-2">
                    <div className="text-sm p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      ...
                      <span className="text-blue-800">{qa.question}</span>
                    </div>
            ...
            {followUpError && (
              <div className="flex items-center space-x-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <span className="text-sm text-red-700">{followUpError}</span>
              </div>
            )}
