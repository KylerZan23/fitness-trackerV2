...
export function AddGoalModal({ isOpen, onClose, onGoalAdded }: AddGoalModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}> {/* Ensure close on overlay click etc. */}
      <DialogContent className="sm:max-w-[425px]">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="metricType" className="text-right">Metric</Label>
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="targetValue" className="text-right">Target ({targetUnit})</Label>
            <Input 
              id="targetValue" 
              type="number" 
              value={targetValue} 
          ...
          {/* Display Form Error */}
          {formError && (
            <p className="text-sm text-red-600 col-span-4 text-center">{formError}</p>
          )}
