...
function generateKeyInsights(data: WeeklyMuscleComparisonItem[]): string[] {
  ...
  const significantIncrease = [...data]
    .filter(
      item =>
        item.percentageChange > 0 && item.percentageChange !== Infinity && item.previousWeekSets > 0
    )
  ...
  const stoppedActivities = data.filter(
    item => item.currentWeekSets === 0 && item.previousWeekSets > 0
  )
  const noChangeActivities = data.filter(
    item => item.changeInSets === 0 && item.currentWeekSets > 0
  )
  ...
  if (insights.length === 0 && data.length > 0) {
    insights.push(
      'Overall training patterns remained similar to last week with no major shifts detected across muscle groups.'
    )
  }
...
export function WeeklyComparison({
  ...
  if (
    !data ||
    data.filter(item => item.currentWeekSets > 0 || item.previousWeekSets > 0).length === 0
  ) {
    return (
      <div className="mt-8 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <p className="text-center py-8 text-gray-500">
          No workout data available for this period to generate a comparison.
        </p>
      </div>
    )
  }
  ...
  const preferredOrder: MuscleGroup[] = [
    MuscleGroup.OTHER,
    MuscleGroup.CHEST,
  ...
  const sortedData = [...relevantData].sort((a, b) => {
    ...
    } else if (trendGroupA === 2) {
      // Neutral trend
      return getPreferredOrderIndex(a.muscleGroup) - getPreferredOrderIndex(b.muscleGroup)
    } else {
      // Downward trend (trendGroupA === 3)
