...
export function Sidebar({ userName = 'User', userEmail, profilePictureUrl, onLogout, className = '' }: SidebarProps) {
  return (
    // Fixed sidebar, light background, dark text, padding, width, hidden on small screens (md:flex)
    <aside className={`fixed top-0 left-0 z-40 w-64 h-screen bg-white text-gray-900 border-r border-gray-200 hidden md:flex flex-col ${className}`}>
      {/* Sidebar Content */}
      <div className="flex flex-col h-full">
        ...
        <div className="p-4 flex flex-col items-center border-b border-gray-200">
          ...
          {userEmail && <p className="text-sm text-gray-500">{userEmail}</p>} {/* Lighter gray text */}
        </div>
        ...
        <nav className="flex-grow p-4 space-y-2">
          {navItems.map((item) => (
            ...
            >
              <item.icon className="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" /> {/* Icon color */}
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="p-4 mt-auto">
          <Button
            ...
            onClick={onLogout}
          >
            <LogOut className="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" /> {/* Icon color */}
            Logout
          </Button>
        </div>
      </div>
