...
export function RunCard({
  ...
  return (
    <div
      className={`bg-white/5 backdrop-blur-sm rounded-xl overflow-hidden border border-white/10 ${className}`}
    >
      {/* User Info Header */}
      ...
      {/* Map */}
      {polyline && <RunMap polyline={polyline} height={isExpanded ? '400px' : '200px'} />}
      ...
      {segment_efforts && segment_efforts.length > 0 && (
        <div className="p-4 border-t border-white/10">
          ...
          {!isExpanded &&
            segment_efforts.slice(0, 1).map(effort => (
              <div key={effort.id} className="flex items-center mb-2">
                <div className="w-8 h-8 bg-yellow-500 text-black rounded-full flex items-center justify-center mr-3">
                  ...
                  >
                    <path
                      fillRule="evenodd"
                      d="M6 3.75A.75.75 0 016.75 3h6.5a.75.75 0 010 1.5h-6.5A.75.75 0 016 3.75zM6 8.25a.75.75 0 01.75-.75h6.5a.75.75 0 010 1.5h-6.5a.75.75 0 01-.75-.75zM5.75 12a.75.75 0 01.75-.75h6.5a.75.75 0 010 1.5h-6.5a.75.75 0 01-.75-.75z"
                      clipRule="evenodd"
                    />
                  </svg>
                </div>
                <div>
                  ...
                  <div className="text-sm text-gray-500">
                    {formatTime(effort.elapsed_time)} {effort.pr_rank === 1 && '• PR'}
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="p-4 border-t border-white/10 flex justify-end">
        <button onClick={toggleExpanded} className="text-sm text-gray-500 hover:text-gray-700">
