...
export const StravaConnect = ({ userId, onConnectionChange }: StravaConnectProps) => {
  ...
  const handleConnect = () => {
    try {
      // Use the exact numeric client ID directly
      const clientId = 154490; // Your Strava client ID as a number
      const redirectUri = `${window.location.origin}/run-logger/callback`;
      
      // Construct the URL with the hardcoded numeric client ID
      const authUrl = `https://www.strava.com/oauth/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=read,activity:read,activity:write&approval_prompt=auto`;
      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 mt-2">Your Strava account is connected and ready to log runs.</p>
          </div>
          <button
  ...
  return (
    <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 mb-8">
      <div className="text-center py-6">
        <svg viewBox="0 0 24 24" className="h-12 w-12 text-orange-500 mx-auto mb-4" fill="currentColor">
          <path d="M15.387 17.944l-2.089-4.116h-3.065L15.387 24l5.15-10.172h-3.066m-7.008-5.599l2.836 5.598h4.172L10.463 0 5 13.828h4.172" />
        </svg>
