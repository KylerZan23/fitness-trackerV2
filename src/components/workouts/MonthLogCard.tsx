    ...
    <Link href={href} className="block group">
      ...
      <div className="bg-gray-50 p-4 h-48 relative flex flex-col justify-between group-hover:shadow-md transition-shadow duration-150">
        ...
        <div className="absolute top-4 left-4 z-10">
          <p className="text-4xl font-bold text-gray-800 group-hover:text-indigo-600">
            {totalDurationHours}
          </p>
        ...
        <p className="absolute top-4 right-4 text-sm font-medium text-gray-400 uppercase z-10 group-hover:text-indigo-600">
          {monthName}
        </p>
        ...
        <div className="mt-auto h-2/5 pt-2 flex items-end overflow-hidden relative">
          {workouts.length === 0 ? (
            <p className="text-xs text-gray-400 w-full text-center self-center">
              No workouts logged.
            </p>
          ) : (
            workouts.map(workout => {
              ...
              try {
                ...
                if (
                  ...
                  console.warn(
                    `Workout ${workout.id} date ${workout.created_at} outside expected range ${year}-${monthIndex}. Skipping bar.`
                  )
                  return null
                }
                ...
                barHeightPercent = Math.max(
                  5,
                  Math.min(100, (workout.duration / maxDurationMinutes) * 100)
                ) // Ensure min height 5%
              } catch (e) {
                console.error(
                  `Error processing date for workout ${workout.id}: ${workout.created_at}`,
                  e
                )
                return null
              }
