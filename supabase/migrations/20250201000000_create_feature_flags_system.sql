-- Migration: Create Feature Flags System for Phoenix Pipeline Rollout
...
CREATE POLICY "Only admins can modify feature flags"
    ...
    USING (
        EXISTS (
            ...
            WHERE id = auth.uid() 
            AND (email LIKE '%@neurallift.ai' OR is_admin = true)
        )
    );
...
INSERT INTO public.feature_flags (
    ...
    FALSE,
    0,
    '{"created_for": "Phoenix pipeline rollout", "documentation": "docs/implementation_plans/phoenix-feature-flagging-system.md"}'::jsonb
...
-- Grant execute permission on the utility function to authenticated users
